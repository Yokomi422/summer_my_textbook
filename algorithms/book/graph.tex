...
ナイーブなUnionFind木は最悪の場合ノードが線形リストのように連結されるため、計算量がO(N)になります。この問題を解決するために、以下の2つによる効率化を行います。
\begin{itemize}
  \item 経路圧縮
  \item ランク基準のマージ
\end{itemize}

\textbf{経路圧縮}とは根を探す際に再帰的に経由したノードを根に直接つなぐことで、木の高さを低くすることです。
下の例では、Dの根を探索するときに、D->C->B->Aと経路を辿っていきますが、 D、C、Bすべての直接の親をAに変更することで、
根を探索する際に高さを低くすることができます。
...
\begin{center}
  \begin{tikzpicture}[scale=0.8]
    ...
    \node[circle, draw, minimum size=1.2cm] (D) at (-3, -4) {D};

    \draw[<-] (A) -- (B);
    \draw[<-] (B) -- (C);
    \draw[<-] (C) -- (D);

    \node[circle, draw, minimum size=1.2cm] (A) at (6, 2) {A};
    \node[circle, draw, minimum size=1.2cm] (B) at (3, 0) {B};
    \node[circle, draw, minimum size=1.2cm] (C) at (5, 0) {C};
    \node[circle, draw, minimum size=1.2cm] (D) at (8, 0) {D};
    ...
    % 経路圧縮前と経路圧縮後を描く
    \node at (-0.8, -5.2) {経路圧縮前};
    \node at (5.9, -5.2) {経路圧縮後};
  \end{tikzpicture}
\end{center}

\textbf{ランク基準のマージ}によって、UnionFind木にそれぞれのノードの高さをrankとして持たせ、高さが低い木を高い木にマージすることで、木の高さを低くすることができます。
高い方にマージすることによってマージ後も木の高さが変わりません。ただし、高さが同じ場合は高さが1増えます。

例えば、DとFをそれぞれ含む木を結合するとき、DとFの根を調べてからそのrankを基準にしてマージを行います。rankの小さい方をrankの大きい方の木の
根に繋げます。
\vspace{0.5cm}
...
\begin{center}
  \begin{tikzpicture}[scale=0.8]
    % 左側の木 (ランク2の木)
    \node[circle, draw, minimum size=1.2cm] (A1) at (-2, 2) {A};
    \node[circle, draw, minimum size=1.2cm] (B1) at (-3, 0) {B};
    \node[circle, draw, minimum size=1.2cm] (C1) at (-1, 0) {C};
    \node[circle, draw, minimum size=1.2cm] (D1) at (-2, -2) {D};
    ...
    % 右側の木 (ランク1の木)
    \node[circle, draw, minimum size=1.2cm] (A2) at (4, 2) {E};
    \node[circle, draw, minimum size=1.2cm] (B2) at (4, 0) {F};
    ...
    % マージ後の木
    \node[circle, draw, minimum size=1.2cm] (A3) at (10, 2) {A};
    \node[circle, draw, minimum size=1.2cm] (B3) at (9, 0) {B};
    \node[circle, draw, minimum size=1.2cm] (C3) at (11, 0) {C};
    \node[circle, draw, minimum size=1.2cm] (D3) at (9, -2) {D};
    \node[circle, draw, minimum size=1.2cm] (E3) at (14, 0) {E};
    \node[circle, draw, minimum size=1.2cm] (F3) at (14, -2) {F};
